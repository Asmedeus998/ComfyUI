...
| **Motion Description** | Flowing prose motion beats with body-part precision per segment |
| **Environment** | Spatial setting, time of day, lighting, atmosphere — must support the commercial mood |
| **Camera Work** | Shot type, movement, perspective — commercial editing language |
| **Audio Cues** | Music mood, ambient sound, SFX, voiceover tone |
| **Reference Integration** | Explicitly maps `@ImageN` to prompt elements with parenthetical nouns |
...
4. **Prompt Structure — Three-Section Flowing Prose Format (Per Segment):**
   Each segment prompt is divided into three distinct sections. Keep each segment under ~250 words.

   **Section 1 — Global Basic Settings (concise prose, 2-3 sentences):**
   - Ad type classification for the segment.
   - Subject lock: brief mention that character is locked to `@Image1 (character)`. Do NOT write a long prose description of the character — let `@Image1 (character)` carry the visual weight.
   - Product lock: brief mention that product is locked to `@Image5 (product)` (or whichever slot holds the product). Do NOT describe the product in excessive detail — reference `@Image5 (product)`.
   - Environment: brief mention of setting, locked to `@Image4 (environment)` or `@Image7 (creative)` as appropriate.
   - Aesthetic style: color palette, mood, film references — drawn from `@Image6 (style)` and `@Image7 (creative)`.
   ...
   **Section 2 — Time Slice Storyboard (single flowing paragraph, semicolon-separated beats):**
   - A single paragraph of flowing prose divided into four time ranges: **0-3s:**, **3-7s:**, **7-11s:**, **11-15s:**.
   - Within each time range, describe motion beats separated by **semicolons (`;`)**.
   - Each beat should describe: body part + action + facial expression + camera note.
   - **Only 1 camera movement per time slice** — do not stack multiple camera directions within a single range.
   - Product interactions must specify which hand and how.
   - Embed inline audio cues as `{audio description}` within the relevant beat.
   - Keep motion descriptions natural and flowing — avoid robotic step-by-step breakdowns.
   - Example: `0-3s: Character walks into frame from left; right hand brushes hair back; {upbeat intro music swells}; camera holds medium shot. 3-7s: Left hand reaches for product on counter; eyes glance toward camera with subtle smile; camera slow push-in to close-up.`

   **Section 3 — Constraints:**
   - End each segment with exactly this constraint block:
...
6. **Reference Integration Protocol**:
   - **ALWAYS refer to images using `@ImageN (noun)` syntax** (`@Image1 (character)`, `@Image2 (costume)`, `@Image7 (creative)`, etc.), never by batch position.
   - Character appearance is visually locked to `@Image1 (character)`. Mention `@Image1 (character)` in the subject lock and at least once per segment to reinforce the visual anchor.
   - Product is visually locked to `@Image5 (product)` (or whichever slot contains the product). Mention `@Image5 (product)` when the product appears.
   - Environment is visually locked to `@Image4 (environment)` or `@Image7 (creative)`. Mention the relevant `@Image` when describing the setting.
   - When `@Image7 (creative)` is provided, adopt its color palette, lighting mood, and compositional energy across ALL time slices in BOTH segments.
...
3. **NO TIMESTAMPED LINES**: NEVER use per-second timestamps like "00:00", "00:01", or "00:15.0". Use the four time ranges "0-3s:", "3-7s:", "7-11s:", "11-15s:" instead.
...
6. **MANDATORY COVERAGE**: Each segment must include: ad type classification, subject description with reference locks, product description with reference locks, commercial environment and aesthetic, flowing time slice storyboard with semicolon-separated beats, camera work, inline audio cues, and constraints block.
7. **CONSISTENCY LOCK**: Character appearance, outfit, and hair must be identical across every time slice in both segments. Product must look the same whenever it appears.
8. **MANDATORY CONTINUE**: Segment 2's opening beat MUST begin with `CONTINUE:` and have NO timestamp prefix.

## PROHIBITIONS
- NEVER output arc labels like "DREAM SETUP", "PRODUCT INTEGRATION", "CTA", "HOOK", "PAYOFF", "SEGMENT 1", "SEGMENT 2" inside the prompt body.
- NEVER use per-second timestamped lines like "00:00" or "00:15.0".
...
- NEVER use vague placeholders like "beautiful scene" or "high quality." Be specific about what the character does in each time slice.
...
- NEVER ignore the reference images/video. Every visual detail from references must be locked into the corresponding time slices.
...
- NEVER omit the commercial narrative arc. The arc must guide your internal timing, but the output must be pure flowing prose beats in the time slice storyboard.
```
...
- `@Image1 (character)`: Character / subject reference — use `@Image1 (character)` for character appearance lock
- `@Image2 (costume)`: Costume / outfit / product reference — use `@Image2 (costume)` for outfit/product lock
- `@Image3 (prop)`: Prop / accessory / secondary subject reference — use `@Image3 (prop)` when relevant
- `@Image4 (environment)`: Environment / scene / background reference — use `@Image4 (environment)` for environment lock
...
- Segment 1's final time slice (11-15s) must end with the character interacting with the product.
- Segment 2's opening beat MUST begin with "CONTINUE:" and describe the exact same pose, hand positions, and product placement as Segment 1's ending. NO timestamp prefix on CONTINUE.
- If `@Image8 (continuation)` is provided, describe the literal pose visible in that frame. Do not invent a new pose.
- Character appearance is locked to `@Image1 (character)` across both segments. Product is locked to `@Image5 (product)` across both segments.
- Keep Section 1 prose concise — use `@ImageN (noun)` references rather than long descriptions.
- Section 2 is a single flowing paragraph with four time ranges (0-3s, 3-7s, 7-11s, 11-15s) and semicolon-separated beats.
- Use inline audio `{audio description}` within beats.
- Only 1 camera movement per time slice.
- End with the constraints block.

Output format: Two segments, each with Section 1 (concise prose with `@Image` locks), Section 2 (flowing time slice storyboard), and Section 3 (constraints). NO arc labels anywhere. Wrap segments in [[SEGMENT_1]] / [[/SEGMENT_1]] and [[SEGMENT_2]] / [[/SEGMENT_2]] tags.
```
...
CRITICAL CONTINUITY INSTRUCTION:
- Segment 1's final time slice (11-15s) must end with the character naturally interacting with the product in the aspirational environment.
- Segment 2's opening beat MUST begin with "CONTINUE:" and describe the exact same pose, hand positions, and product placement as Segment 1's ending. NO timestamp prefix on CONTINUE.
- If `@Image8 (continuation)` is provided, describe the literal pose visible in that frame. Do not invent a new pose.
- Character appearance is locked to `@Image1 (character)` across both segments. Product is locked to `@Image5 (product)` across both segments.
- Keep Section 1 prose concise — use `@ImageN (noun)` references rather than long descriptions.
...
CRITICAL CONTINUITY INSTRUCTION:
- Segment 1's final time slice (11-15s) must end at the dramatic climax — the character interacting with the product at the peak moment.
- Segment 2's opening beat MUST begin with "CONTINUE:" and describe the exact same pose, hand positions, and product placement as Segment 1's ending, continuing the reaction/expression. NO timestamp prefix on CONTINUE.
- If `@Image8 (continuation)` is provided, describe the literal pose visible in that frame. Do not invent a new pose.
- Character appearance is locked to `@Image1 (character)` across both segments. Product is locked to `@Image5 (product)` across both segments.
- Keep Section 1 prose concise — use `@ImageN (noun)` references rather than long descriptions.
- Section 2 is a single flowing paragraph with four time ranges (0-3s, 3-7s, 7-11s, 11-15s) and semicolon-separated beats.
...
**Fix:** Segment 2 MUST begin with `CONTINUE:` and explicitly describe the continuation pose, hand positions, and product placement from Segment 1's final time slice.

### Segment 2 Drift (Reference Amnesia)

**Symptom:** Segment 2's character face, hair, or outfit slowly morphs because the prompt stops referencing `@Image1 (character)`.  
**Fix:** Re-lock character to `@Image1 (character)` in Segment 2's Section 1 prose. Mention the same outfit details, hair style, and distinguishing features.
...
**Fix:** Use the four time ranges (0-3s, 3-7s, 7-11s, 11-15s) with semicolon-separated beats. Each beat should specify body part, action, expression, and camera.
...
**Symptom:** Time slice lines still include arc labels like "0-3s: DREAM SETUP: character turns..."  
...
**Fix:** Demand explicit product lock at specific time slices: "7-11s: Right hand lifts amber glass bottle with blue label toward camera; label faces lens; hero lighting catches glass; product locked to @Image5 (product)".

### Missing Camera Direction

**Symptom:** No camera notes; Seedance defaults to static medium shots.  
**Fix:** Embed one camera note per time slice: "camera slow push-in from wide to close-up", "orbit around product begins", "handheld shake intensifies".
...
[Segment 1 — Section 1: Setup prose with subject/product/environment locks] +
[Segment 1 — Section 2: 0-3s / 3-7s / 7-11s / 11-15s flowing storyboard with semicolon beats, inline audio, one camera per slice] +
[Segment 1 — Section 3: Constraints block] +
[Segment 2 — Section 1: Continuation prose with re-locked references] +
[Segment 2 — Section 2: CONTINUE: + 0-3s / 3-7s / 7-11s / 11-15s flowing storyboard with semicolon beats, inline audio, one camera per slice] +
[Segment 2 — Section 3: Constraints block]
```
