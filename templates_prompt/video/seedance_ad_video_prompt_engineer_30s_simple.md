...
Each generation produces **two 15-second segments** with continuity locks. This simplified version replaces rigid 1-second timestamps with **natural-language scene descriptions**, which Seedance handles better for motion interpolation.
...
- When rigid timestamp prompts produce robotic or uncanny motion
...
| **Subject Lock** | Brief mention that character is locked to `@Image1`. Let the image carry the visual weight. |
| **Product Lock** | Brief mention that product is locked to `@Image2` (or whichever slot). Do NOT describe in excessive detail. |
| **Scene Description** | Flowing natural-language paragraphs describing the scene, action, and camera work. NOT timestamps. |
| **Environment** | Spatial setting, time of day, lighting, atmosphere — supporting the commercial mood |
| **Camera Work** | General camera approach embedded in scene description (e.g., "slow push-in from wide to intimate close-up") |
| **Audio Cues** | Music mood, ambient sound, SFX, voiceover tone |
| **Reference Integration** | Explicitly maps array-position `@ImageN` to prompt elements |
...
2. **ARRAY-POSITION REFERENCE RULE (CRITICAL — DO NOT IGNORE)**:
   ...
   - **You MUST map slot labels to array positions** and use array-position `@ImageN` syntax in your output:
     - The **first image** in the batch (regardless of its slot label) is `@Image1`
     - The **second image** in the batch is `@Image2`
     - The **third image** in the batch is `@Image3`
     - And so on.
   - **NEVER use slot label numbers** like `@Image5` or `@Image7` in the final prompt. If the image labeled "5-PRODUCT" happens to be the 2nd image in the batch, you MUST write `@Image2`.
   - Example: "Character appearance locked to @Image1" or "Product packaging matches @Image2 exactly" — where the number matches the batch position, not the slot label.
   - **VISUAL CONTENT OVERRIDE**: Do not blindly trust slot labels. Analyze the actual visual content of each image. If a slot labeled "5-PRODUCT" contains only text, a watermark, a logo, or otherwise does not show the actual product visually, do NOT describe it as the product reference. Instead, identify which image(s) actually contain the product, character, or environment visuals, and assign the `@ImageN` reference accordingly.
...
4. **Prompt Structure — Two Segments, Each with Natural-Language Commercial Format:**
   ...
   **Part 1 — Commercial Setup (concise prose, 1 short paragraph per segment):**
   ...
   - Product lock: brief mention that product is locked to `@Image2` (or whichever array position holds the product). Do NOT describe the product in excessive detail — reference `@Image2`.
   - Environment: brief mention of setting, locked to `@Image3` or `@Image1` as appropriate.
   ...
   **Part 2 — Scene Description (flowing natural-language paragraphs — NO TIMESTAMPS):**
   - Write 2-4 flowing paragraphs describing the scene, character actions, camera movement, and pacing.
   - Describe motion as **intent and mood**, not micromanaged keyframes. Seedance interpolates motion naturally from natural language.
   - Embed camera notes into the prose: "The camera opens on a medium shot and gradually pushes toward intimate close-ups using smooth, elegant beauty-commercial movement."
   - Describe actions as continuous flows: "She gently examines her skin in the window reflection, then walks slowly through the conservatory, pausing by a sunbeam where she discovers the product resting on a marble pedestal."
   - Product interactions must specify which hand and how, but keep it flowing: "With her right hand, she lifts the amber bottle, examining the elegant packaging before dipping her finger into the rich cream."
   - Keep descriptions natural and cinematic — avoid robotic step-by-step breakdowns.

   **Audio Cues (final paragraph):**
   - Music mood, ambient sound, SFX, diegetic product sounds, voiceover tone.

5. **CONTINUITY PROTOCOL (CRITICAL)**:
   - Segment 2's **very first sentence** MUST begin with the word `CONTINUE:` followed by an explicit description of the character's pose, hand positions, facial expression, and product placement as a **direct continuation** of Segment 1's final moment.
...
6. **Reference Integration Protocol**:
   - **ALWAYS refer to images using array-position `@ImageN` syntax** (`@Image1`, `@Image2`, `@Image3`), matching their position in the batch — never by slot label.
   - Character appearance is visually locked to `@Image1`. Mention `@Image1` in the subject lock and at least once per segment.
   - Product is visually locked to `@Image2` (or whichever position contains it). Mention `@Image2` when the product appears.
   - Environment is visually locked to `@Image3` (or `@Image1` if the same image contains environment).
...
3. **NO TIMESTAMPS**: NEVER use "00:00", "00:01", "0-4s:", or any time-based markers. Use flowing prose paragraphs only.
4. **NO COARSE TIME BLOCKS**: NEVER write "From 0 to 4 seconds..." or "First 5 seconds...". Motion must be described as continuous natural-language scenes.
...
7. **MANDATORY COVERAGE**: Each segment must include: ad type classification, subject description with reference locks, product description with reference locks, commercial environment and aesthetic, flowing scene description with embedded camera work, and audio cues.
8. **CONSISTENCY LOCK**: Character appearance, outfit, and hair must be identical across every part of both segments. Product must look the same whenever it appears.
9. **MANDATORY CONTINUE**: Segment 2's opening sentence MUST begin with `CONTINUE:`.

## PROHIBITIONS
- NEVER output arc labels like "DREAM SETUP", "PRODUCT INTEGRATION", "CTA", "HOOK", "PAYOFF", "SEGMENT 1", "SEGMENT 2" inside the prompt body.
- NEVER use timestamps, time blocks, or second-by-second breakdowns.
...
- NEVER omit the commercial narrative arc. The arc must guide your internal timing, but the output must be pure flowing prose.
```
...
- If the first image is a composite storyboard/grid, reference it as @Image1 for all visual elements it contains.

CRITICAL CONTINUITY INSTRUCTION:
- Segment 1's final moment must end with the character interacting with the product.
- Segment 2's very first sentence MUST begin with "CONTINUE:" and describe the exact same pose, hand positions, and product placement as Segment 1's ending.
- If an 8-LAST continuation frame is provided, describe the literal pose visible in that frame.
- Character appearance is locked to @Image1 across both segments. Product is locked to @Image2 across both segments.
- Keep Part 1 prose concise — use @ImageN references rather than long descriptions.
- Part 2 must use FLOWING NATURAL-LANGUAGE PARAGRAPHS. NO timestamps. NO "00:00" or "0-4s" blocks. Describe scenes cinematically, like a director's brief, not a shot list.

Output format: Two segments, each with Part 1 (concise prose with @Image locks) and Part 2 (flowing scene description in paragraphs). NO arc labels anywhere. NO timestamps anywhere. Wrap segments in [[SEGMENT_1]] / [[/SEGMENT_1]] and [[SEGMENT_2]] / [[/SEGMENT_2]] tags.
```
...
- Segment 2's very first sentence MUST begin with "CONTINUE:" and describe the exact same pose, hand positions, and product placement as Segment 1's ending.
- If an 8-LAST continuation frame is provided, describe the literal pose visible in that frame.
- Character must match @Image1 exactly across both segments. Product must match @Image2 exactly across both segments.
- Part 2 must use FLOWING NATURAL-LANGUAGE PARAGRAPHS. NO timestamps. Describe the scene cinematically.

Output format: Two segments, each with Part 1 (concise prose) and Part 2 (flowing paragraphs). NO arc labels. NO timestamps. Wrap segments in [[SEGMENT_1]] / [[/SEGMENT_1]] and [[SEGMENT_2]] / [[/SEGMENT_2]] tags.
```
...
- Segment 2's very first sentence MUST begin with "CONTINUE:" and describe the exact same pose, hand positions, and product placement as Segment 1's ending, continuing the reaction/expression.
- If an 8-LAST continuation frame is provided, describe the literal pose visible in that frame.
- Character must match @Image1 exactly across both segments. Product must match @Image2 exactly across both segments.
...
**Fix:** Segment 2's opening sentence MUST begin with `CONTINUE:` and explicitly describe the continuation pose, hand positions, and product placement from Segment 1's ending.
...
**Fix:** Re-lock character to `@Image1` in Segment 2's Part 1 prose. Mention the same outfit details, hair style, and distinguishing features.

### Timestamp Relapse

**Symptom:** The prompt slips back into rigid timestamp format: "00:00 character stands... 00:01 she leans..." This gives Seedance no natural motion interpolation.  
**Fix:** Demand flowing prose paragraphs. Describe scenes like a director's brief, not a shot list.
...
**Fix:** Always count by batch position: 1st image = `@Image1`, 2nd = `@Image2`, 3rd = `@Image3`.
...
**Fix:** Embed camera notes into prose: "The camera slowly pushes in from a wide establishing shot to an intimate close-up, then begins a gentle orbit around the product."
...
[Segment 1 — Part 2: Flowing scene description in 2-4 paragraphs with embedded camera and action] +
