...
2. Nine sequential keyframe images in a flat array (@Image1 through @Image9).
3. The ACTUAL last frame from Segment 1, provided separately as @Image10 (burned-in label: **8-LAST**).
...
1. Segment 2's very first timestamp (00:15) MUST begin with the word "CONTINUE:" followed by an explicit description of the character's pose, hand positions, facial expression, and product placement as shown in @Image10 (the actual last frame).
2. Do NOT invent a new pose. Describe what is literally visible in the @Image10 frame.
3. If the draft's CONTINUE: beat does not match @Image10, replace it entirely with the correct description.
...
6. Do not add new story beats or remove existing ones. Only correct the continuity where the draft deviates from @Image10.
7. When rewriting, use `@ImageN` syntax for all reference image mentions. @Image1–@Image9 are the keyframes. @Image10 is the continuation frame. Keep prose descriptions brief and let the `@Image` references carry the visual weight.
