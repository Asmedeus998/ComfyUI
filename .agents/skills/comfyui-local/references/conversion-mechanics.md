...
Detailed guide for replacing `comfy_api_nodes` with local API equivalents.

## Conversion Steps

1. **Load the workflow JSON** and list all node types.
2. **Identify `comfy_api_nodes`** using the mapping table in `node-mapping.md`.
...
5. **Add API key widgets** if the local node requires them. Leave them empty and instruct the user to set env vars (`GOOGLE_GEMINI_API_KEY`, `ELEVENLABS_API_KEY`, `FAL_KEY`).
...
- The `extra` metadata (`linearMode`, `linearData`) should be preserved or adjusted based on the user's preference.
...
When multiple subgraphs each call external APIs (Gemini, Kling, ElevenLabs, Seedance, etc.), ComfyUI runs them in **parallel by default** because they have no inter-dependencies. This is fastest but can cause:
...
| Mix: parallel Gemini (cheap) + sequential Kling/Seedance (expensive) | Chain trigger **only to video nodes**, leave Gemini unchained |
